<?php
/*
Possíveis diferenças entre este campo e o UploadArquivo:

no onPosPost, o fieldUpload deleta o arquivo da pasta temp.
			o UploadArquivo faz nada.
			
no getTempFile o fileUpload seta um nome com uma extensão "_".time()
			o UploadArquivo não põe a extensão por tempo
			
na rotina para deletar o fileUpload não verifica se é insert ou update e não retira o nome da session do arquivo
			o UploadArquivo verifica que tipo de rotina é no form e retira o nome da session no caso de ser insert.
*/
class FieldUpload extends Field {
	...
	function FieldUpload($tmp_params) {
		parent::Field("upload", $tmp_params[0]);
